...
struct ShowReplyFixture : Fixture {
  Session wired{link, fs, hash, sys, outbox, &ui};
...
TEST(Session, CapsAdvertiseBulkTransferAndListsActions) {
  EXPECT_TRUE(Session::kCaps & caps::kBulkTransfer);
  EXPECT_TRUE(Session::kCaps & caps::kListsActions);
  // Lanes that have not landed must stay off, or the phone will send SetCards
  // and get a Nack it did not expect.
  EXPECT_FALSE(Session::kCaps & caps::kCards);
